import { ArrowRight, CalendarDays, FolderOpen, Music2, Trash2 } from "lucide-react";
...
export default function ProjectCard({ project, compact = false, onDelete, deleting = false }) {
  ...
  return (
    <article className="group relative overflow-hidden rounded-lg border border-white/10 bg-gradient-to-br from-white/[0.07] via-white/[0.04] to-black/30 p-5 shadow-[0_18px_55px_rgba(0,0,0,0.24)] transition duration-200 hover:-translate-y-0.5 hover:border-teal-200/30 hover:bg-white/[0.07]">
        ...
        <div className="flex items-center gap-2">
          {onDelete ? (
            <button
              ...
              disabled={deleting}
              className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-rose-300/15 bg-rose-400/10 text-rose-100 transition hover:border-rose-300/30 hover:bg-rose-400/20 disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <Link to={`/projects/${project.id}`} className="inline-flex items-center gap-1 text-xs font-semibold uppercase tracking-[0.14em] text-zinc-500 transition hover:text-teal-100">
            Open
            <ArrowRight size={13} />
          </Link>
        </div>
